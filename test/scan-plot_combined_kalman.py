...
## kalman inialization
kmf = KalmanFilter (dim_x=1, dim_z=1)
kmf.x =  np.array([[-70.]])
kmf.F = np.array([[1.0]])
kmf.H = np.array([[2.0]])
kmf.P = 5.0
kmf.Q = 0.07
#kmf.Q = Q_discrete_white_noise(dim=2, dt=0.1, var=0.07)
kmf.R = 3.0
...
def exit_handler():
	...
	os.system("sudo hciconfig hci2 down")
...
def submit_to_server():
	...
	if len(fp) != 0:
		for mac in fp:
			...
			# 	plotLenCount=0
			# plt.scatter(plotLenCount, kmf.x, c=args.color)
			fingerprints2.append({"mac": mac, "rssi": int(median / div)})
	else :
		kmf.update(np.array([float(-108)]))
				...
				if temp['rssi'] != 'invalid':
					...
					with fingerprints_lock:
						...
						if temp['mac'] == "04:a3:16:97:d9:c8":
							...
							# kmf.predict()
							kmf.update(np.array([float(temp['rssi'])]))
